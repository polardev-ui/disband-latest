...
async function upload(request, env) {
  ...
  try {
    ...
    key = `${auth.userId}/${crypto.randomUUID()}.${extensionOf(file.name, type)}`;
    const url = `${env.PUBLIC_BASE}/v1/images/${key}`;

    // Keep the live user-scoped key and quota contract while hashing the upload.
    const [toStore, toHash] = file.stream().tee();
    const digestStream = new crypto.DigestStream("SHA-256");
    hashing = toHash.pipeTo(digestStream);
...
/** Preserve a blocked object outside public media routes for later review. */
async function quarantine(request, env) {
  const secret = request.headers.get("x-admin-secret");
  if (!env.ADMIN_SECRET || !secret || !constantTimeEqual(secret, env.ADMIN_SECRET)) {
    return json({ error: "Unauthorized" }, 401, env);
  }
  const { key, sha256 } = await request.json().catch(() => ({}));
  const mediaKey = /^[0-9a-f-]{36}(?:\/[0-9a-f-]{36})?\.[a-z0-9]+$/i;
  if (typeof key !== "string" || !mediaKey.test(key) || typeof sha256 !== "string" || !/^[0-9a-f]{64}$/i.test(sha256)) {
    return json({ error: "Invalid key or sha256" }, 400, env);
  }
 ...
 *
 * The prefix cannot match the flat or user-scoped UUID keys accepted by
 * `serve()`, so cache objects are unreachable through /v1/images/.
 */
