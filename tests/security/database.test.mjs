...
test('security migration: server-only privileges survive the mascot teardown; tenure stays idempotent', async () => {
  ...
  try {
    ...
    // Teardown drops the mascot surface; re-run it to prove it is idempotent.
    await db.exec(await readFile('supabase/migrations/0071_remove_mascots.sql','utf8'));
    await db.exec(await readFile('supabase/migrations/0071_remove_mascots.sql','utf8'));
    // Mascot tables and functions (including the trigger function, dropped
    // last) are gone. The storage bucket is removed via the Storage API, not
    // SQL, so it is not asserted here.
    ...
    // Server-only functions keep their hardened privileges from 0064.
    ...
    // Subscription tenure accrual is still present and idempotent.
...
test('referrals: code minting, email-verified counting, self/dupe guards, leaderboard', async () => {
  ...
  try {
    ...
    // Production wires this trigger up in 0001; the function body under test
    // is 0072's redefinition.
    ...
    // The referrer signs up first (no code of their own to use).
    ...
    // A friend signs up with the code but never confirms their email yet.
    ...
    // Banned referrers fall off the board.
    ...
    // An unknown code is ignored; that signup simply carries no referral.
    ...
    // One referral per account, ever — and never yourself.
    ...
    // Privileges: the leaderboard is public, the internals are not.
